...
def analyze_device_fingerprint(device_id: str, merchant_id: str) -> dict:
    ...
    return {
        ...
        "risk_note": "Perangkat tidak dikenal — risiko tinggi"
        if not is_known
        else "Perangkat dikenal — aman",
    }
...
def get_merchant_profile(merchant_id: str) -> dict:
    ...
    return profiles.get(
        merchant_id,
        {"name": "Unknown", "type": "unknown", "city": "Unknown", "owner": "Pemilik"},
    )
...
class MonitorAgent:
    ...
    def evaluate(self, transaction: dict, ml_result: dict) -> dict:
        ...
        if score >= self.threshold_block:
            ...
            trigger_reason = (
                f"Skor risiko sangat tinggi ({score:.2f}). Langsung blokir."
            )
        elif score >= self.threshold_flag:
            action = "TRIGGER_INVESTIGATE"
            trigger_reason = (
                f"Skor risiko mencurigakan ({score:.2f}). Perlu investigasi."
            )
    ...
    def investigate(self, transaction: dict, ml_result: dict) -> dict:
        ...
        if pattern_match["pattern_count"] > 0:
            evidence_flags.append(
                f"POLA_FRAUD_TERDETEKSI({pattern_match['pattern_count']})"
            )
        if transaction.get("amount_vs_avg_ratio", 1) > 5:
            evidence_flags.append("JUMLAH_TIDAK_WAJAR")

        confidence = min(
            0.5 + (len(evidence_flags) * 0.15) + ml_result["ensemble_score"] * 0.3, 1.0
        )
...
class ResolutionAgent:
    ...
    def resolve(
        ...
        if monitor_action == "TRIGGER_BLOCK" or confidence >= 0.80:
            final_action = "BLOCK"
            action_detail = (
                "Transaksi diblokir otomatis. Terlalu banyak indikator fraud."
            )
        ...
        reasoning = self._generate_reasoning(
            transaction, ml_result, investigation, final_action
        )
...
class CommunicatorAgent:
    ...
    def notify(self, transaction: dict, resolution: dict, investigation: dict) -> dict:
        ...
        message = self._generate_notification(
            owner_name, action, amount, location, txn_id, resolution, investigation
        )
        ...
        notification = {
            ...
            "priority": "HIGH"
            if action == "BLOCK"
            else ("MEDIUM" if action == "STEP_UP_AUTH" else "LOW"),
    ...
    def _generate_notification(
        self, owner_name, action, amount, location, txn_id, resolution, investigation
    ):
    ...
    def process(self, transaction: dict, ml_result: dict) -> dict:
        ...
        if monitor_result["action"] in ["TRIGGER_INVESTIGATE", "TRIGGER_BLOCK"]:
            ...
            investigation = {
                ...
                "merchant_profile": get_merchant_profile(merchant_id),
                "history_summary": {
                    "total_transactions": len(TRANSACTION_HISTORY.get(merchant_id, [])),
                    "avg_amount": 0,
                    "max_amount": 0,
                },
        ...
        resolution = self.resolution.resolve(
            transaction, ml_result, monitor_result, investigation
        )
