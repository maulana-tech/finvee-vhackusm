...
CITY_COORDS = {
    ...
    "Solo":       (-7.5755,  110.8243), "Semarang":   (-6.9932, 110.4203),
    "Yogyakarta": (-7.7956,  110.3695), "Makassar":   (-5.1477, 119.4327),
    "Palembang":  (-2.9761,  104.7754), "Denpasar":   (-8.6705, 115.2126),
    "Malang":     (-7.9797,  112.6304), "Pekanbaru":  (0.5071,  101.4478),
    "Balikpapan": (-1.2675,  116.8289), "Pontianak":  (-0.0263, 109.3425),
    "Manado":     (1.4748,   124.8421), "Padang":     (-0.9471, 100.4172),
    "Banjarmasin":(-3.3186,  114.5944), "Mataram":    (-8.5833, 116.1167),
}
MERCHANTS = {
...
def make_gauge(score, title="Risk Score"):
    ...
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            ...
            "bgcolor": "#1a1d2e", "bordercolor": "#333",
            "steps": [
...
def action_badge(action):
    if action == "BLOCK":       return '<span class="badge-block">🚫 BLOCKED</span>'
    elif action == "STEP_UP_AUTH": return '<span class="badge-stepup">🔐 STEP-UP</span>'
    else:                       return '<span class="badge-approve">✅ APPROVED</span>'
...
def normalize_city(loc):
    ...
    if loc_str in CITY_COORDS: return loc_str
    for city in CITY_COORDS:
        if city.lower() in loc_str.lower(): return city
    return "Jakarta"

def risk_color(level):
    return {"CRITICAL":"#E74C3C","HIGH":"#E67E22","MEDIUM":"#F39C12","LOW":"#27AE60"}.get(level,"#aaa")
...
with st.sidebar:
    ...
    c2.metric("Alerts", len([n for n in st.session_state.notifications if n.get("priority") in ["HIGH","MEDIUM"]]))
        ...
        for i in range(10):
            ...
            if is_fraud:
                txn = {
                    ...
                    "is_new_device": 1, "transaction_count_1h": random.randint(8,18),
...
with tab1:
    ...
    with col_form:
        with st.form("analyze_form"):
            ...
            merchant_id   = c2.selectbox("Merchant", list(MERCHANTS.keys()),
                                          format_func=lambda k: f"{k} — {MERCHANTS[k]['name']}")
            amount        = c1.number_input("Amount (Rp)", min_value=0.0, value=500000.0, step=10000.0)
            merchant_type = c2.selectbox("Type", ["food","retail","service","grocery","beauty"])
    ...
    with col_result:
        if submitted:
            payload = {
                "transaction_id": txn_id, "merchant_id": merchant_id,
                "merchant_type": merchant_type, "amount": amount,
                "hour": hour, "day_of_week": 1, "location": location,
                "device_id": "DEV_NEW" if is_new_device else "DEV_0001",
                "is_new_device": int(is_new_device),
                "transaction_count_1h": txn_1h, "transaction_count_24h": txn_24h,
...
with tab2:
    ...
    st.markdown("Upload file **CSV** atau **Excel** berisi transaksi untuk dianalisis secara massal.")
...
| `transaction_count_24h` | | Frekuensi 24 jam |
        """)
        with open("/home/ubuntu/aegis-sme/data/sample_transactions_template.csv", "rb") as f:
            st.download_button("⬇️ Download Template CSV", f.read(),
                               "aegis_template.csv", "text/csv", use_container_width=True)
        ...
        if uploaded_file is not None:
            ...
            try:
                ...
                if st.button("🚀 Analyze All Transactions", use_container_width=True, type="primary"):
                    ...
                    results_list = []
                    defaults = {
                        "transaction_id": None, "merchant_id": "UKM001",
                        "merchant_type": "W", "amount": 100.0, "hour": 12,
                        "day_of_week": 1, "location": "Jakarta",
                        "device_id": "DEV_0001", "is_new_device": 0,
                        "transaction_count_1h": 2, "transaction_count_24h": 8,
                        "amount_vs_avg_ratio": 1.0, "location_mismatch": 0,
                    }
                    for i, (_, row) in enumerate(df_upload.iterrows()):
                        ...
                        for col, default in defaults.items():
                            if col in row and pd.notna(row[col]):
                                ...
                                txn[col] = f"TXN_UPLOAD_{i:04d}" if col == "transaction_id" else default
                        ...
                        try:
                            ...
                            results_list.append({
                                ...
                                "amount_vs_avg_ratio":  float(txn["amount_vs_avg_ratio"]),
                                "transaction_count_1h": int(txn["transaction_count_1h"]),
                            ...
                            results_list.append({
                                ...
                                "day_of_week": 1, "is_new_device": 0,
                                "location_mismatch": 0, "amount_vs_avg_ratio": 1.0,
                                "transaction_count_1h": 1,
                                "lgb_score": 0.0, "ensemble_score": 0.0,
                                "decision": "ERROR", "risk_level": "UNKNOWN",
                                "explanation": str(e),
                            })
                        progress.progress((i+1)/len(df_upload), text=f"Analyzing {i+1}/{len(df_upload)}...")

                    st.session_state.batch_results = pd.DataFrame(results_list)
                    progress.empty()
                    st.success(f"✅ Analisis selesai! {len(results_list)} transaksi diproses.")
                    st.rerun()
            except Exception as e:
    ...
    if st.session_state.batch_results is not None:
        ...
        m2.metric("🚫 Diblokir",        f"{blocked}",  f"{blocked/total*100:.1f}%",  delta_color="inverse")
        m3.metric("⚠️ Step-Up Auth",    f"{step_up}",  f"{step_up/total*100:.1f}%",  delta_color="off")
        ...
        with col_tbl:
            ...
            disp = df_res[["transaction_id","merchant_id","location","amount","ensemble_score","decision","risk_level"]].copy()
        ...
        fig_hist = px.histogram(
            df_res, x="ensemble_score", nbins=30, color="decision",
            color_discrete_map={"BLOCK":"#E74C3C","STEP_UP_AUTH":"#E67E22","APPROVE":"#27AE60","ERROR":"#888"},
        ...
        st.download_button("⬇️ Export Hasil ke CSV", csv_export,
...
with tab3:
    ...
    map_source = st.radio("Sumber Data", ["Generate Sample Data","Batch Upload Results","Demo Simulation"], horizontal=True)

    map_df = None
    if map_source == "Batch Upload Results" and st.session_state.batch_results is not None:
        ...
        for c in st.session_state.cases:
            rows.append({
                ...
                "amount_vs_avg_ratio":  15.0 if c.get("final_action")=="BLOCK" else 1.5,
                "transaction_count_1h": 15 if c.get("final_action")=="BLOCK" else 2,
        ...
        for i in range(300):
            ...
            sample_rows.append({
                ...
                "location":             city,
                "amount":               random.uniform(5000,15000) if is_fraud else random.uniform(50,2000),
                "hour":                 random.choice([1,2,3,4]) if is_fraud else random.randint(8,20),
                "day_of_week":          random.randint(0,6),
                "is_new_device":        1 if is_fraud else 0,
                "location_mismatch":    1 if is_fraud else 0,
                "amount_vs_avg_ratio":  random.uniform(10,25) if is_fraud else random.uniform(0.5,2.5),
                "transaction_count_1h": random.randint(10,20) if is_fraud else random.randint(1,5),
                "lgb_score":            random.uniform(0.3,0.9) if is_fraud else random.uniform(0.01,0.15),
                "ensemble_score":       random.uniform(0.4,0.95) if is_fraud else random.uniform(0.02,0.2),
                "decision":             "BLOCK" if is_fraud and random.random()>0.3 else ("STEP_UP_AUTH" if is_fraud else "APPROVE"),
    ...
    if map_df is not None and not map_df.empty:
        ...
        city_sum = map_df.groupby("city").agg(
        ...
        city_sum["fraud_rate"]      = (city_sum["blocked"] / city_sum["total"] * 100).round(1)
        city_sum["suspicious_rate"] = ((city_sum["blocked"]+city_sum["step_up"]) / city_sum["total"] * 100).round(1)
        city_sum["lat"] = city_sum["city"].map(lambda c: CITY_COORDS.get(c,(-6.2,106.8))[0])
        city_sum["lon"] = city_sum["city"].map(lambda c: CITY_COORDS.get(c,(-6.2,106.8))[1])

        col_map, col_rank = st.columns([3,1])
        with col_map:
            fig_map = px.scatter_mapbox(
                city_sum, lat="lat", lon="lon",
                size="total", color="fraud_rate",
                color_continuous_scale=["#27AE60","#F39C12","#E67E22","#E74C3C"],
                range_color=[0,100],
                hover_name="city",
                hover_data={"total":True,"blocked":True,"fraud_rate":True,"avg_score":":.4f","lat":False,"lon":False},
                size_max=50, zoom=4,
                center={"lat":-2.5,"lon":118.0},
                mapbox_style="carto-darkmatter",
                title="🗺️ Fraud Distribution Map — Indonesia",
                labels={"fraud_rate":"Fraud Rate (%)","total":"Total Txn","blocked":"Blocked"}
            )
            fig_map.update_layout(
                height=480,
                paper_bgcolor="rgba(0,0,0,0)", font=dict(color="#ccc"),
                coloraxis_colorbar=dict(title="Fraud Rate %", tickfont=dict(color="#ccc")),
        ...
        with col_rank:
            ...
            top_cities = city_sum.nlargest(8,"blocked")[["city","total","blocked","fraud_rate"]]
            for _, row in top_cities.iterrows():
                c = "#E74C3C" if row["fraud_rate"]>50 else ("#E67E22" if row["fraud_rate"]>25 else "#F39C12")
                st.markdown(f"""
        ...
        with col_bar:
            fig_bar = px.bar(
                ...
                x="fraud_rate", y="city", orientation="h",
                color="fraud_rate",
                color_continuous_scale=["#27AE60","#F39C12","#E67E22","#E74C3C"],
        ...
        with col_stack:
            pivot = map_df.groupby(["city","decision"]).size().unstack(fill_value=0).reset_index()
            for col in ["APPROVE","STEP_UP_AUTH","BLOCK"]:
                if col not in pivot.columns: pivot[col] = 0
            pivot_melted = pivot.melt(id_vars="city", value_vars=["APPROVE","STEP_UP_AUTH","BLOCK"])
            pivot_melted = pivot_melted.rename(columns={"variable": "decision"})
            fig_stack = px.bar(
                pivot_melted,
                x="city", y="value", color="decision",
                color_discrete_map={"BLOCK":"#E74C3C","STEP_UP_AUTH":"#E67E22","APPROVE":"#27AE60"},
...
with tab4:
    ...
    st.markdown("Analisis tren fraud berdasarkan waktu — kapan, seberapa sering, dan seberapa besar.")

    track_source = st.radio("Sumber Data", ["Generate Sample Data","Batch Upload Results"], horizontal=True, key="track_src")

    track_df = None
    if track_source == "Batch Upload Results" and st.session_state.batch_results is not None:
        ...
        for i in range(500):
            ...
            hour = random.choice([1,2,3,23]) if is_fraud else random.choices(
                range(24), weights=[1,1,1,1,1,2,3,5,8,10,10,9,8,8,9,10,10,9,8,6,5,4,3,2]
            )[0]
            rows.append({
                "transaction_id":       f"T{i:04d}",
                "merchant_id":          f"UKM{random.randint(1,5):03d}",
                "location":             random.choice(CITIES),
                "amount":               random.uniform(3000,15000) if is_fraud else random.uniform(50,2000),
                "hour":                 hour,
                "day_of_week":          random.randint(0,6),
                "is_new_device":        1 if is_fraud else 0,
                "location_mismatch":    1 if is_fraud else 0,
                "amount_vs_avg_ratio":  random.uniform(10,25) if is_fraud else random.uniform(0.5,3),
                "transaction_count_1h": random.randint(10,20) if is_fraud else random.randint(1,5),
                "lgb_score":            random.uniform(0.3,0.9) if is_fraud else random.uniform(0.01,0.15),
                "ensemble_score":       random.uniform(0.4,0.95) if is_fraud else random.uniform(0.02,0.2),
                "decision":             "BLOCK" if is_fraud and random.random()>0.2 else ("STEP_UP_AUTH" if is_fraud else "APPROVE"),
                "risk_level":           "CRITICAL" if is_fraud else "LOW",
                "explanation":          "",
            })
    ...
    if track_df is not None and not track_df.empty:
        ...
        with col_h:
            hourly = track_df.groupby("hour").agg(
                total=("transaction_id","count"),
                blocked=("decision", lambda x: (x=="BLOCK").sum()),
                avg_score=("ensemble_score","mean"),
            ).reset_index()
            ...
            fig_h.add_trace(go.Bar(x=hourly["hour"],y=hourly["total"],name="Total",
                                   marker_color="#1e3a5f",opacity=0.8), secondary_y=False)
            fig_h.add_trace(go.Scatter(x=hourly["hour"],y=hourly["fraud_rate"],name="Fraud Rate %",
                                       line=dict(color="#E74C3C",width=2.5),
                                       mode="lines+markers",marker=dict(size=6)), secondary_y=True)
            fig_h.update_layout(title="Transaksi & Fraud Rate per Jam", height=300, **PLOTLY_DARK,
                                 legend=dict(orientation="h",y=-0.25))
            fig_h.update_xaxes(title_text="Jam", gridcolor="#2a2d3e")
            fig_h.update_yaxes(title_text="Jumlah", secondary_y=False, gridcolor="#2a2d3e")
            fig_h.update_yaxes(title_text="Fraud Rate %", secondary_y=True, gridcolor="#2a2d3e")
            st.plotly_chart(fig_h, use_container_width=True)

        with col_d:
            daily = track_df.groupby("day_of_week").agg(
                total=("transaction_id","count"),
                blocked=("decision", lambda x: (x=="BLOCK").sum()),
            ).reset_index()
            ...
            fig_d.add_trace(go.Bar(x=daily["day_name"],y=daily["total"],name="Total",
                                   marker_color="#1e3a5f",opacity=0.8), secondary_y=False)
            fig_d.add_trace(go.Scatter(x=daily["day_name"],y=daily["fraud_rate"],name="Fraud Rate %",
                                       line=dict(color="#E67E22",width=2.5),
                                       mode="lines+markers",marker=dict(size=6)), secondary_y=True)
            fig_d.update_layout(title="Transaksi & Fraud Rate per Hari", height=300, **PLOTLY_DARK,
                                 legend=dict(orientation="h",y=-0.25))
            fig_d.update_xaxes(title_text="Hari", gridcolor="#2a2d3e")
            fig_d.update_yaxes(title_text="Jumlah", secondary_y=False, gridcolor="#2a2d3e")
            fig_d.update_yaxes(title_text="Fraud Rate %", secondary_y=True, gridcolor="#2a2d3e")
            st.plotly_chart(fig_d, use_container_width=True)

        # Heatmap Hour × Day
        st.markdown("#### 🔥 Fraud Heatmap — Jam × Hari")
        hm = track_df[track_df["decision"]=="BLOCK"].groupby(["day_of_week","hour"]).size().reset_index(name="count")
        hm_pivot = hm.pivot(index="day_of_week",columns="hour",values="count").fillna(0)
        hm_pivot.index = [DAY_NAMES.get(i,str(i)) for i in hm_pivot.index]
        for h in range(24):
            if h not in hm_pivot.columns: hm_pivot[h] = 0
        ...
        fig_hm = px.imshow(hm_pivot,
            ...
            labels={"x":"Jam","y":"Hari","color":"Jumlah Fraud"}, aspect="auto")
        ...
        with col_v:
            fig_vio = px.violin(track_df, y="ensemble_score", x="decision", color="decision",
                color_discrete_map={"BLOCK":"#E74C3C","STEP_UP_AUTH":"#E67E22","APPROVE":"#27AE60"},
                title="Distribusi Risk Score per Keputusan", box=True, points="outliers")
            fig_vio.update_layout(height=320, **PLOTLY_DARK, showlegend=False)
            st.plotly_chart(fig_vio, use_container_width=True)

        with col_s:
            fig_sc = px.scatter(track_df.sample(min(300,len(track_df))),
                x="amount", y="ensemble_score", color="decision",
                color_discrete_map={"BLOCK":"#E74C3C","STEP_UP_AUTH":"#E67E22","APPROVE":"#27AE60"},
                title="Amount vs Risk Score",
                labels={"amount":"Jumlah (Rp)","ensemble_score":"Risk Score"}, opacity=0.7)
        ...
        top10 = track_df.nlargest(10,"ensemble_score")[
            ["transaction_id","merchant_id","location","amount","hour","ensemble_score","decision","risk_level"]
        ].copy()
...
with tab5:
    ...
    if not st.session_state.cases:
        ...
        m2.metric("🚫 Blocked",  blocked,  f"{blocked/len(cases)*100:.1f}%", delta_color="inverse")
        m3.metric("⚠️ Step-Up",  step_up,  f"{step_up/len(cases)*100:.1f}%", delta_color="off")
        ...
        with col_pie2:
            fig2 = px.pie(values=[blocked,step_up,approved],
                          names=["Blocked","Step-Up Auth","Approved"],
                          color_discrete_sequence=["#E74C3C","#E67E22","#27AE60"],
        ...
        with col_bar2:
            df_cases = pd.DataFrame([{
                "id": c["case_id"], "score": c["risk_score"], "action": c["final_action"]
            } for c in cases[-15:]])
            fig_scores = px.bar(df_cases, x="id", y="score", color="action",
                color_discrete_map={"BLOCK":"#E74C3C","STEP_UP_AUTH":"#E67E22","APPROVE":"#27AE60"},
...
with tab6:
    ...
    if not st.session_state.cases:
        ...
        sel_case = next((c for c in st.session_state.cases if c["case_id"] == sel_id), None)

        if sel_case:
            action = sel_case.get("final_action","APPROVE")
            color  = {"BLOCK":"#E74C3C","STEP_UP_AUTH":"#E67E22","APPROVE":"#27AE60"}.get(action,"#aaa")
            st.markdown(f"""
            ...
            agents_info = [
                ("🔔 Monitor Agent",       "Mendeteksi anomali & trigger investigasi",    "#4A90D9"),
                ("🕵️ Investigator Agent",  "5 tool calls: device, lokasi, pola, riwayat", "#8E44AD"),
                ("⚖️ Resolution Agent",    "LLM reasoning → keputusan final",              color),
                ("📱 Communicator Agent",  "Notifikasi personal ke pemilik UKM",           "#27AE60"),
            ]
            cols = st.columns(4)
            ...
            steps = [
                ("🔔 1. Monitor Agent",       "#4A90D9", f"**Action:** `{mon.get('action','N/A')}`\n\n**Reason:** {mon.get('trigger_reason','N/A')}"),
                ("🕵️ 2. Investigator Agent",  "#8E44AD", f"**Tools:** {', '.join([f'`{t}`' for t in inv.get('tools_called',[])])}\n\n**Device:** {'⚠️ Unknown' if not inv.get('device_check',{}).get('is_known_device') else '✅ Known'} | **Location:** {'⚠️ Mismatch' if inv.get('location_check',{}).get('mismatch') else '✅ OK'}\n\n**Fraud Confidence:** {inv.get('fraud_confidence',0):.0%}"),
                ("⚖️ 3. Resolution Agent",    color,     f"**Decision:** `{res.get('final_action','N/A')}`\n\n> {res.get('reasoning','N/A')}"),
                ("📱 4. Communicator Agent",  "#27AE60", f"**Recipient:** {notif.get('recipient','N/A')} | **Priority:** `{notif.get('priority','N/A')}`\n\n> {notif.get('message','N/A')}"),
            ]
...
with tab7:
    ...
    if not st.session_state.notifications:
        st.info("ℹ️ Belum ada notifikasi. Jalankan Demo Simulation atau Analyze Transaction.")
    else:
        filter_p = st.multiselect("Filter Prioritas", ["HIGH","MEDIUM","LOW"], default=["HIGH","MEDIUM","LOW"])
        filtered = [n for n in reversed(st.session_state.notifications) if n.get("priority") in filter_p]
